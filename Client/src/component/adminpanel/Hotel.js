...
const Hotel = () => {
  ...
  const handleUpdate = async (hotelId) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/hotels/getHotel/${hotelId}`
      );
      setUpdateHotel(response.data);
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await axios.put(
      `http://localhost:8080/hotels/update/${updateHotel.hotelId} `,
      updateHotel
    );
    console.log("after call update api :" + response.data);
    const updatedHotels = await axios.get(
      "http://localhost:8080/hotels/getAllHotels"
    );
  ...
  return (
    <div className="hotelMainContainer">
      <button className="addhotelbtn" onClick={() => setModel(true)}>
        Add Hotel
      </button>
              ...
              <div className="hotel-details">
                ...
                <div className="hotel-info">
                  <span className="hotel-price">
                    Rs {value.pricePerNight} / night
                  </span>
                </div>
              </div>
              <button
                className="updatebutton"
                onClick={() => handleUpdate(value.hotelId)}
              >
                Update
              </button>
              <button
                className="deletebutton"
                onClick={() => handledelete(value.hotelId)}
              >
                Delete
              </button>
            </div>
